...
func (a torprojectExits) Fetch(logger *zap.SugaredLogger, client *http.Client, blocklistURL *url.URL) ([]*net.IPNet, error) {
	var ret []*net.IPNet
	...
	for scanner.Scan() {
		...
		if len(line) == 0 {
			...
			continue
		} else if !strings.HasPrefix(line, "ExitAddress") {
			continue
		}
